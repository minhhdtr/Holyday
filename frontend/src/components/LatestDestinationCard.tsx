...
const LatestDestinationCard = ({ hotel }: Props) => {
  return (
    <Link to={`/detail/${hotel._id}`} className='relative cursor-pointer overflow-hidden rounded-md'>
      ...
      <div className='absolute bottom-0 p-4 bg-black opacity-75 w-full rounded-b-md'>
        <span className='text-white font-bold tracking-tight text-2xl'>{hotel.name}</span>
      </div>
    </Link>
  );
};
